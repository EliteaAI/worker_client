...
class Method:  # pylint: disable=E1101,R0903,W0201
    ...
    def llm_invoke(  # pylint: disable=R0913
        ...
        llm_interface_callback = self.llm_interface["llm_invoke_callback"]
        #
        ...
        if use_legacy_llm_mode or llm_interface_callback is None:
            ...
            #
            callback = self.integrations[integration_name]["llm_invoke_callback"]
            #
            if callback is None:
    ...
    def chat_model_invoke(  # pylint: disable=R0913
        ...
        """ Call model """
        use_legacy_llm_mode = self.descriptor.config.get("use_legacy_llm_mode", False)
        llm_interface_callback = self.llm_interface["chat_model_invoke_callback"]
        #
        if integration_name not in self.llm_interface["supported_integrations"]:
            use_legacy_llm_mode = True
        #
        ...
        if use_legacy_llm_mode or llm_interface_callback is None:
            if integration_name not in self.integrations:
                raise RuntimeError("Unknown integration")
            #
            callback = self.integrations[integration_name]["chat_model_invoke_callback"]
            #
            if callback is None:
                raise RuntimeError("Action is not supported")
            #
            ...
            #
            invoke_kwargs.update(callback(settings, messages))
            #
            task_id = self.task_node.start_task(
                name="invoke_model",
                kwargs=invoke_kwargs,
        ...
        #     raise RuntimeError("LLM action is not supported")
        #
        return llm_interface_callback(integration_name, settings, messages)
